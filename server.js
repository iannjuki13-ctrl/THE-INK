// server.js — THE INK News Backend
...
// --- Fetch RSS items only (no summary yet) ---
  ...
  const feedPromises = feeds.map(async url=>{
    try{
      ...
      return (feed.items || []).slice(0,8).map(it=>({
        ...
        raw: it,
        source: sourceTitle,
        summary: '(Loading summary...)', // placeholder
        image: pickImageFromItem(it, 0)
      }));
    }catch(err){
...
async function updateSummaries(items){
  const tasks = items.map((it,idx)=> async ()=>{
    ...
    try{
      it.summary = await summarizeWithOpenAI(prompt);
    }catch(e){
      it.summary = '(Summary unavailable)';
    }
  });
  await batchMap(tasks, fn=>fn(), 3, 300);
}

// --- Fetch & cache ---
async function fetchAndCache(){
  const items = await fetchFeedItems(HIDDEN_FEEDS);
  cachedItems = items;
  lastUpdated = Date.now();
  updateSummaries(cachedItems); // async, background
  return cachedItems;
}

// --- Endpoints ---

// GET auto-fetch hidden RSS feeds
...
app.post('/api/fetchSummaries', async (req,res)=>{
  ...
  cachedItems = items;
  lastUpdated = Date.now();
  updateSummaries(cachedItems); // async
  res.json({ items });
});
