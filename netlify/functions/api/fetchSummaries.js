...
async function summarizeWithOpenAI(prompt) {
  const res = await fetch('https://api.openai.com/v1/chat/completions',{
    ...
    body: JSON.stringify({
      model: 'gpt-4o-mini',
      messages: [{role:'system', content:'You are a professional news editor. Summarize in 80-120 words, neutral tone, include why it matters. End with "Source: [source] — Read full article here: [link]"'}],
      temperature: 0.3,
      max_tokens: 350
    })
  });
...
Task: Summarize this news article in your own words, neutral tone. Include Why it matters. End with "Source: ${item.source} — Read full article here: ${item.link}".
`;
      let summary='';
